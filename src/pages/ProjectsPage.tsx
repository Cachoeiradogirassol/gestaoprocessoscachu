...
export default function ProjectsPage() {
  ...
  const [newProject, setNewProject] = useState({ name: '', description: '', start_date: '', end_date: '', responsible_id: '' });
  ...
  const createProject = async () => {
    ...
    if (error) {
      ...
      setNewProject({ name: '', description: '', start_date: '', end_date: '', responsible_id: '' });
  ...
  const updateProject = async () => {
    ...
    if (!error) {
      ...
      fetchProjects();
      // Notify participants
      const { data: participants } = await supabase.from('event_participants').select('user_id').eq('event_id', editingProject.id);
      if (participants) {
        for (const p of participants) {
          if (p.user_id !== user?.id) {
            await supabase.from('notifications').insert({
              user_id: p.user_id, type: 'project_updated',
              title: 'Projeto atualizado', message: editingProject.name, link: `/projects/${editingProject.id}`,
            });
          }
        }
      }
  ...
  const deleteProject = async (proj: Project) => {
    ...
    const { data: participants } = await supabase.from('event_participants').select('user_id').eq('event_id', proj.id);
    if (participants) {
      for (const p of participants) {
        if (p.user_id !== user?.id) {
          await supabase.from('notifications').insert({
            user_id: p.user_id, type: 'project_deleted',
            title: 'Projeto excluído', message: proj.name, link: '/projects',
          });
        }
      }
    }
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex items-center gap-4 text-xs text-muted-foreground">
                  {formatDateRange(proj) && (
                    <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{formatDateRange(proj)}</span>
                  )}
                  <span className="flex items-center gap-1"><Users className="h-3 w-3" />{getProfileName(proj.responsible_id)}</span>
                </div>
              </CardContent>
            </Card>
          ))}
